	...
	void *parent_payload)
{
	...
	assert(oid && repo && tree && parent_cb);
...
static const git_oid *commit_parent_from_oids(size_t curr, void *payload)
{
	...
	const git_tree *tree)
{
	...
	if (!tree) {
		...
		git_oid_cpy(&tree_oid, git_tree_id(old_tree));
		git_tree_free(old_tree);
	} else {
		assert(git_tree_owner(tree) == repo);
		git_oid_cpy(&tree_oid, git_tree_id(tree));
	}

	return git_commit_create_from_callback(
		id, repo, update_ref, author, committer, message_encoding, message,
		&tree_oid, commit_parent_for_amend, (void *)commit_to_amend);
}
